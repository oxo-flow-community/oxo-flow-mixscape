...
# Ported from upstream workflow/scripts/visualize.R (epigen/mixscape_seurat
...
#   2  CalcPerturbSig.gene_col
#   3  CalcPerturbSig.nt_term
#   4  RunMixscape.prtb_type
#   5  CalcPerturbSig.split_by_col
#   6  RunMixscape.split_by_col
#   7  Antibody_Capture ("" disables the antibody-expression block)
...
antibody_capture_flag <- args[7]

# outputs (same names as upstream; derived from the sample result dir)
prtb_score_plot_path <- file.path(sample_dir, "plots", "PerturbScore")
post_prob_plot_path <- file.path(sample_dir, "plots", "PosteriorProbability")
ab_expr_plot_path <- file.path(sample_dir, "plots", paste0(antibody_capture_flag, "_expression"))
